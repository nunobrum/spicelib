...
* __Reading Simulation Wave Files__
  A tool that can read .raw files generated by LTSpice, NGSpice, QSpice and other Spice like simulators that are 
  inspired on the original Spice .raw format from Berkeley university. See[RawRead](#rawread)

* __Netlist Editor__
  A tool that can read and edit a Spice netlist. This allows the user to update component values, simulation primitives
  and parameters. 

* __Simulation Batching__
  A tool that allows to launch simulations in parallel thus better exploiting the available computing power.
