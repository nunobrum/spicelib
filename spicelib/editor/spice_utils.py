...
lib_inc_regex = re.compile(
    r"^\.(LIB|INC(LUDE)?)\s+(?P<filename>.+?)\s*(?:[;*].*)?$",
    re.IGNORECASE
)
...
REPLACE_REGEXS : Dict[str, str] = {
    'A': PREFIX_AND_NODES_RGX("A", 8, 8) + r"\s+(?P<value>.*)",  # LTspice Only : Special Functions, Parameter substitution not supported
    # Bxxx n001 n002 [VIRP]=<expression> [ic=<value>] ...
    'B': PREFIX_AND_NODES_RGX("B", 2) + r"\s+(?P<value>[VIBR]\s*=(?:>=|<=|[^=])+)" + PARAM_RGX,  # Behavioral source
